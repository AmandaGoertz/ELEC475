...
class AdaIN_net(nn.Module):
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        if self.training:  # training
            ...
            content_enc = self.encode(content)
            style_enc = self.encode(style)

            target = self.adain(content_enc[-1], style_enc[-1])
            target = alpha * target + (1 - alpha) * content_enc[-1]

            target_dec = self.decode(target)
            target_enc = self.encode(target_dec)

            # Eq 12
            loss_c = self.content_loss(target_enc[-1], target)
            # Eq 13
            loss_s = self.style_loss(target_enc[0], style_enc[0])

            for i in range(1, 4):
                loss_s += self.style_loss(target_enc[i], style_enc[i])
            ...
            target = self.adain(content_enc[-1], style_enc[-1])
            target = alpha * target + (1 - alpha) * content_enc
